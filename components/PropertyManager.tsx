...
const PropertyEditor = ({ property, onSave, onDelete }: { property: LocalProperty, onSave: (updatedProperty: LocalProperty) => void, onDelete: () => void }) => {
  const [localProp, setLocalProp] = useState<LocalProperty>(() => ({
      ...property,
      // Ensure options array exists for select types
  ...
  const handleTypeChange = (v: string) => {
    ...
    if (newType === 'SELECT' || newType === 'MULTI_SELECT') {
        // Initialize options if switching to a select type
        setLocalProp(p => ({ ...p, type: newType, options: p.options ?? [] }));
    } else {
        // Clear options if switching away
        ...
        <div className="grid gap-2">
          ...
          <div className="grid grid-cols-3 items-center gap-4">
            ...
            <Select value={localProp.type} onValueChange={handleTypeChange}>
              ...
              <SelectContent>
                {propertyTypes.map(pt => <SelectItem key={pt} value={pt}>{pt.toLowerCase().replace('_', ' ')}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          {renderOptionEditor()}
...
export const PropertyManager = ({ properties, onPropertiesChange }: PropertyManagerProps) => {
  ...
  const handleUpdateProperty = (updatedProperty: LocalProperty) => {
    // If the type changed away from a select type, we might need to clear the value.
    if (updatedProperty.type !== 'SELECT' && updatedProperty.type !== 'MULTI_SELECT') {
        ...
        if (existingProp && (existingProp.type === 'SELECT' || existingProp.type === 'MULTI_SELECT')) {
            updatedProperty.value = ''; // Reset value if type changed
        }
    }
    ...
    switch (prop.type) {
      case 'SELECT':
        return (
          <Select
            value={prop.value || ''}
            onValueChange={(value) => handleValueChange(prop.id, value)}
          >
            ...
            <SelectContent>
              {(prop.options && prop.options.length > 0) ? (
                ...
                <div className="text-xs text-muted-foreground p-2">no options defined.</div>
              )}
            </SelectContent>
          </Select>
        );
